...
export default function ProductCard(props) {
  ...
  return (
    <div className='w-full flex flex-col items-center justify-center gap-[16px] h-[288px] px-[4px] sm:px-[8px] py-[16px] border-2 border-light-background rounded-md shadow-md'>
      <img className='cursor-pointer w-[120px] h-[120px]' src={props?.image?.includes('https://') ? props?.image : `https://thilaa.jethitech.com/storage/${props?.image}`} action="product image" onClick={()=>{setModalProductId(props.p_id); props.openProduct(true);setSelectededProduct(product)}}/>
